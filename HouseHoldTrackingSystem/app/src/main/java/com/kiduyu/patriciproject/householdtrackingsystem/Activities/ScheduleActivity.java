...
public class ScheduleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonViewLocation = findViewById(R.id.viewSupplierLocation);
        buttonViewLocation.setOnClickListener(v -> {
            String supplier = sItems.getSelectedItem().toString();
            if (supplier.equals("Choose a delivery Agent")){
                FancyToast.makeText(getBaseContext(),"Choose a supplier first", FancyToast.LENGTH_LONG, FancyToast.ERROR, false).show();
            }else {
                FancyToast.makeText(getBaseContext(),"Loading "+supplier+" Location", FancyToast.LENGTH_LONG, FancyToast.SUCCESS, false).show();
        ...
        schedule.setOnClickListener(v -> {
            if (extras != null) {
                ...
                String spinner1 = sItems.getSelectedItem().toString().trim();
    ...
    private void ScheduleDelivery(String cons_name, String cons_total, String spinner1, String spinner2) {
        if (TextUtils.isEmpty(cons_total)){
            ...
            sendToFirebaseDb(cons_name,cons_total,spinner1,spinner2, progressDialog);
        }
    }

    private void sendToFirebaseDb(String cons_name, String cons_total, String spinner1, String spinner2, ProgressDialog progressDialog){
        final DatabaseReference RootRef;
        RootRef = FirebaseDatabase.getInstance().getReference().child("Delivery Orders");
        ...
        params.put("username", Prevalent.currentOnlineUser.getName());

        RootRef.child(spinner1).push().setValue(params);
        FancyToast.makeText(getApplicationContext(),"Order placed Successfully", FancyToast.LENGTH_LONG,FancyToast.SUCCESS,false).show();
